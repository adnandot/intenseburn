    ...
    <div class="receipysearch">
       ...
       <div style="float:left; width:100%; margin:20px 0px;" class="orange line"></div>     
		...
		<?php 
				...
				$allrecipe = get_user_meta(getLoggedinUserId(),'user_recipe', true);
				
				if(!empty($allrecipe)){
					$allrecipe = @unserialize($allrecipe);
					if(!is_array($allrecipe)){
						$allrecipe = array();
					}
				}
				...
				$userplan          = get_user_meta(getLoggedinUserId(),'silver_plan', true);
					$userargs = array(
						...
						'author__in'        	=> array(getLoggedinUserId()),
					
					);
				$user_recipe = new WP_Query($userargs);
				if($user_recipe->have_posts()){
						while ($user_recipe->have_posts() ) : $user_recipe->the_post();;
							  $post_ids[] = get_the_ID();
							endwhile;
						
							if(!empty($post_ids)){
								$allrecipe = array_merge($allrecipe,$post_ids);
							}
